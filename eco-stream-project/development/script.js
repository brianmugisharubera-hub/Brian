...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Contact form logic for admin dashboard
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            const formMessage = document.getElementById('formMessage');
            formMessage.textContent = 'Request sent';
            formMessage.className = 'success';
            contactForm.reset();
        });

        // Live error clearing
        ['name', 'email', 'service', 'message', 'privacy'].forEach(id => {
    ...
    function displaySubmissions() {
        ...
        // Group submissions by serviceType
        const groups = {};
        submissions.forEach(sub => {
            if (!groups[sub.serviceType]) groups[sub.serviceType] = [];
            groups[sub.serviceType].push(sub);
        });
        let html = '';
        const serviceTypes = ['Express', 'Standard', 'Bulk'];
        serviceTypes.forEach(type => {
            if (groups[type] && groups[type].length > 0) {
                html += `<h3>${escapeHtml(type)} Requests</h3>`;
                html += '<table class="submissions-table"><thead><tr><th>Name</th><th>Email</th><th>Details</th><th>Date</th></tr></thead><tbody>';
                groups[type].slice().reverse().forEach(sub => {
                    html += `<tr><td>${escapeHtml(sub.name)}</td><td>${escapeHtml(sub.email)}</td><td>${escapeHtml(sub.details)}</td><td>${escapeHtml(sub.date)}</td></tr>`;
                });
                html += '</tbody></table>';
            }
        });
